...
if len(thermostats) > 0:
    ...
    for i, thermostat in enumerate(thermostats):
        if "thermostatMode" in thermostat and\
            ...
            if thermostat['thermostatOperatingState'] == 'cooling' or\
            thermostat['thermostatOperatingState'] == 'heating':
                setpointAction = ' to '
            currentSetpoint = 0
            ...
            setpointText = "(" + str(thermostat['thermostatOperatingState']) + setpointAction + str(currentSetpoint) + degree_symbol + ")"
            if "displayName" in thermostat:
                print thermostat['displayName'],setpointText,'|image=', thermoImage
            ...
            if "thermostatMode" in thermostat:
                print "--Mode:",thermostat['thermostatMode']
            if "thermostatOperatingState" in thermostat:
                print "--Status:",thermostat['thermostatOperatingState']
            if "lastOperationEvent" in thermostat:
                timespan = thermostat['lastOperationEvent']
            ...
            if "coolingSetpoint" in thermostat:
                if thermostat['coolingSetpoint'] != None:
            ...
            if "heatingSetpoint" in thermostat:
                if thermostat['heatingSetpoint'] != None:
