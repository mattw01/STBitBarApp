...
# Read Use Images
try:
    useImagesString = re.sub(r'^"|"$', '', config.get('My Section', 'useImages'))
    if useImagesString.lower() == 'true':
    	useImages = True
    else: useImages = False
...
try:
    ...
    pass
try:
    matchOutputNumberOfDecimalsString = re.sub(r'^"|"$', '', config.get('My Section', 'matchOutputNumberOfDecimals'))
    if matchOutputNumberOfDecimalsString.lower() == 'true':
    	matchOutputNumberOfDecimals = True
    else: matchOutputNumberOfDecimals = False
except ConfigParser.NoOptionError:
...
if mainDisplay[0]['name'] == None:
    print formatter.formatNumber(mainDisplay[0]['value']), thermoColor
else:
...
# Output Locks
if len(locks) > 0: print "Locks|font=Helvetica-Bold color=black"
for sensor in locks:
	...
	if sensor['value'] == 'locked':
		...
		sym = '🔓'
		img = redUnlocked
	else: sensor['name'] = sensor['name'] + "(" + sensor['value'] + ")"
	if useImages is True:
