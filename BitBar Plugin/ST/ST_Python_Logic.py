...
# Get the sensor arrays from the JSON data and sort by name
try:
  temps       = sorted(j['Temp Sensors'], key=lambda k: k['name'])
  contacts    = sorted(j['Contact Sensors'], key=lambda k: k['name'])
  switches    = sorted(j['Switches'], key=lambda k: k['name'])
  motion      = sorted(j['Motion Sensors'], key=lambda k: k['name'])
  mainDisplay = sorted(j['MainDisplay'], key=lambda k: k['name'])
  locks       = sorted(j['Locks'], key=lambda k: k['name'])
  presences   = sorted(j['Presence Sensors'], key=lambda k: k['name'])
except KeyError,e:
...
if len(thermostat) > 0:
	...
	if "thermostatMode" in thermostat[0] and\
		...
		setpointText = "(" + str(thermostat[0]['thermostatOperatingState']) + setpointAction + str(currentSetpoint) + ")"
		...
		if "coolingSetpoint" in thermostat[0]:
			...
			print "--Cooling Set Point (" + str(currentCoolingSetPoint) + ")|color=blue"
		...
		if "heatingSetpoint" in thermostat[0]:
			...
			print "--Heating Set Point (" + str(currentHeatingSetPoint) + ")|color=red"
			print "----Change Setpoint|size=9"
...
if countSensors > 0:
    menuTitle = "Temp Sensors ("+str(countSensors)+")"
    print menuTitle, "| font=Helvetica-Bold color=black size=15"
    colorSwitch = False
    ...
    for i, sensor in enumerate(temps):
        ...
        if i >= mainMenuMaxItems:
            if i == mainMenuMaxItems:
                ...
                print "-- " + menuTitle
                print "-- " + sensor['name'], whiteSpace, currentValue+degree_symbol, '|font=Menlo', colorText
...
if countSensors > 0:
    menuTitle = "Contact Sensors ("+str(countSensors)+")"
...
if countSensors > 0:
    menuTitle = "Motion Sensors ("+str(countSensors)+")"
    print menuTitle,"|font=Helvetica-Bold color=black"
...
if countSensors > 0:
    menuTitle = "Presence Sensors (" + str(countSensors) + ")"
    print menuTitle, "|font=Helvetica-Bold color=black"
    ...
    for i, sensor in enumerate(presences):
        ...
        if i >= mainMenuMaxItems:
            if i == mainMenuMaxItems:
                print "{} More... | {}".format(countSensors - mainMenuMaxItems, subMenuMoreColor)
                print "-- " + menuTitle
                print "-- " + sensor['name'], whiteSpace, emoji, '|font=Menlo', colorText
            else:
                print "-- " + sensor['name'], whiteSpace, emoji, '|font=Menlo', colorText
        else:
            print sensor['name'], whiteSpace, emoji, '|font=Menlo', colorText
...
if countSensors > 0:
    menuTitle = "Locks ("+str(countSensors)+")"
    print menuTitle,"|font=Helvetica-Bold color=black"
